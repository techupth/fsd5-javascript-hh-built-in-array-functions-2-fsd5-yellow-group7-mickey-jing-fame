...
for (let i = 0; i < bills.length; i++) {
  if (!locationMember.includes(bills[i].location)) {
    locationMember.push(bills[i].location);
    totalPaidByLocation[`${bills[i].location}`] = bills[i].total;
  } else {
    totalPaidByLocation[`${bills[i].location}`] =
      bills[i].total + totalPaidByLocation[`${bills[i].location}`];
  }
}
console.log(locationMember);
